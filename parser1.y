...
U : RETURN NUM {add('n');} ';'{add('t');/*printf("Return\t%s\n",$2.name);*/ /*insert_icg("return",$2.name,"","");*/} | RETURN ID ';' {add('t');/*printf("Return\t%s\n",$2.name);*//*icg[icc].op="return"; icg[icc].arg1=$2.name; icc++;*/}| ;

S :	IF{add('k');} '(' C ')'{/*printf("\nLabel\t%s:\n",$4.tr);*//*icg[icc].op="Label"; icg[icc].result=$4.tr;icc++;*/}'{'{addTo('{',"Punctuations");} S '}'{addTo('}',"Punctuations");
	pop();
	//printf("goto next\n");
	/*printf("\nLabel\t%s:\n",$4.fal);*/ /*icg[icc].op="Label"; icg[icc].result=$4.fal;icc++;*/} EL {$$.nd=mknode($4.nd,$9.nd,"IF");
	strcpy($$.name,"IF");}
	|assign {$$.nd=$1.nd;}
	...
	|DO {add('k');} '{'{addTo('{',"Punctuations");} S'}'{addTo('}',"Punctuations"); flag=1;} WH 
...
B : 	E relop E {$$.nd=mknode($1.nd,$3.nd,$2.name);
	...
	/*char * tempo=strcat($2.name,$3,name)   
	char *tempo1= strcat($1.name,);
   icg[icc]->op="if";
   icg[icc]->arg1=tempo1;
   icg[icc]->result=strcat("L",ifd);
   icc++;
   icg[icc].op="ifFalse";
   icg[icc].arg1=tempo1;
   icg[icc].result=strcat("L",eld);
   icc++;*/
   sprintf($$.tr,"L%d",ifd);
